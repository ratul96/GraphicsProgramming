...
int main(int argc, char *argv[])
{
	...
	bool cmdTurnRight = false, cmdTurnLeft = false, cmdMoveForward = false, cmdMoveBackward = false;
	...
	while( go )
	{
		...
		if (cmdTurnLeft & !cmdTurnRight)
		{
			myScene.SetTurn(0.01f);
		}
		else if (cmdTurnRight & !cmdTurnLeft)
		{
			myScene.SetTurn(-0.01f);
		}
		else
		{
			myScene.SetTurn(0.0f);
		}
		if (cmdMoveBackward & !cmdMoveForward)
		{
			myScene.SetSpeed(0.5f);
		}
		else if (cmdMoveForward & !cmdMoveBackward)
		{
			myScene.SetSpeed(-0.5f);
		}
		else
		{
			myScene.SetSpeed(0.0f);
		}
